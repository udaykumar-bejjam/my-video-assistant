...
- Smoke tests: `npm test` (includes packs, broll, distribution, trim, full-duration, chess-eval, aspect-remap, heuristic-parity, timeline-snap, chess-pgn-import, **lufs**).
...
- LUFS metering is shared via `enhancer-server/src/lufs.js` ↔ Swift `AudioNormalizeService` (target −14 LUFS).
